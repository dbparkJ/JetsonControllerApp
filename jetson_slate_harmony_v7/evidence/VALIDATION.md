# Slate Harmony V7 — 실제 수행한 검증과 한계
...
| 가로 경계 검사 | 128조합 중 넘침 0 | 32상태 × 412/1.0, 360/1.0, 360/1.3, 360/선형2.0 |
| 대표 상호작용 | 12/12 | 탭·선택·확인·취소·복구·다크 유지; 서버/API 없음 |
| 렌더된 텍스트 | 658개 노드 / 47개 고유 쌍 | DOM의 실제 계산색과 가장 가까운 불투명 조상 배경 |
| 텍스트 대비 | 47/47 쌍 4.5:1 이상 | 최저 4.5588:1; 최종 값 반올림 전 판정 |
| 렌더된 아이콘 | 348개 / 고유 30쌍 | SVG stroke와 인접 배경, 장식 아이콘도 포함 |
| 아이콘 대비 | 30/30 쌍 3:1 이상 | 이것만으로 모든 비텍스트/포커스 감사가 완료되지는 않음 |
| 역할 계약 쌍 | 62/62 | 텍스트 4.5, controlBorder/focusRing 3.0 목표 |
| 브라우저 런타임 오류 | 0 | 검사한 경로 내 |
| 프로토타입 외부 요청 | 0 | 로컬 검사 중 관찰; 장비 접속 없음 |
| Figma JS 구문/모의 검사 | 통과 | 화면 32, variant 28, 변수 171, 텍스트 스타일 38의 생성 경로 |
| 중복 생성 방지/PNG 메시지 | 모의 API에서 통과 | 실제 Figma 생성·export 아님 |
| 패키지 정합성 | PASS | JSON/HTML/Figma 입력/Kotlin/스타일 표/프롬프트 동기화 |
