/** Slate Harmony V7 — local Figma development plugin.
 ...
 */
const DATA = __DATA__;
const VERSION = 'Slate Harmony V7';
...
async function createTokens(){
 const primitives=figma.variables.createVariableCollection('SHV7 / Primitives');
 const raw={};const unique=new Set(Object.values(DATA.themes).flatMap(t=>Object.values(t)));
 for(const hex of unique){let v=figma.variables.createVariable('color/'+hex.slice(1),primitives,'COLOR');v.scopes=[];v.hiddenFromPublishing=true;v.setVariableCodeSyntax('ANDROID','Color(0xFF'+hex.slice(1)+')');v.setValueForMode(primitives.defaultModeId,rgb(hex));raw[hex]=v;}
 // Separate single-mode collections avoid depending on multiple-mode entitlements.
 for(const theme of ['light','dark']){
  let c=figma.variables.createVariableCollection('SHV7 / '+camel(theme));colors[theme]={};
  for(const [role,hex]of Object.entries(DATA.themes[theme])){
   let v=figma.variables.createVariable('color/'+role,c,'COLOR');v.scopes=['FRAME_FILL','SHAPE_FILL','TEXT_FILL','STROKE_COLOR'];v.setValueForMode(c.defaultModeId,figma.variables.createVariableAlias(raw[hex]));v.setVariableCodeSyntax('ANDROID','slateHarmonyTokens(darkTheme).'+role);colors[theme][role]=v;
  }
 }
 const d=figma.variables.createVariableCollection('SHV7 / Dimensions');
...
async function styleFor(snap){
 ...
 const st=figma.createTextStyle();st.name='SHV7/'+(size>=24?'Display':size>=16?'Body':'Label')+'/'+size+'/'+weight;st.fontName=fontNames[weight>=600?700:weight>=500?500:400];st.fontSize=size;st.lineHeight={unit:'PIXELS',value:lh};st.letterSpacing={unit:'PIXELS',value:-.2};textStyles.set(key,st);return st;
}
...
async function createIconMasters(){
 ...
 for(const [name,path]of Object.entries(DATA.icons)){
  const c=track(figma.createComponent());c.name='SHV7/Icon/'+name;c.description='24px outline icon. Change semantic stroke role on the instance; decorative icons have no spoken label in the app.';c.resize(24,24);c.fills=[];
  const svg=track(figma.createNodeFromSvg(`<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#28313B" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">${path}</svg>`));
...
async function createNode(snap,parent,theme,scale=1,master=false){
 ...
 if(snap.component&&!master){
  ...
  if(!entry){
   ...
   n.description='Slate Harmony '+snap.component+'. Editable label, auto layout, semantic colors. Sample state is not device telemetry.';
 ...
 if(snap.action)actionNodes.push({node:n,action:snap.action,theme});
 return n;
}
async function createDocumentation(parent){
 ...
 const texts=[['COBALT SOFT / 2026',16,700],['현장에서 중요한 것을 먼저.',42,700],['딥 네이비 · 코발트 블루 · 아이스 블루 · 따뜻한 오프화이트 / 홈 · 작업 · 데이터 · 설정',20,500],['이 파일은 로컬 플러그인으로 생성한 편집 가능한 디자인입니다. 수치와 상태는 시안용 예시이며 실제 앱 또는 장비 검증 결과가 아닙니다.',16,400],['레이어: Auto Layout · 텍스트 스타일 · 단일 모드 Light/Dark 변수 · Button/Badge variants · SVG 아이콘 인스턴스',14,400]];
...
async function build(){
 ...
 try{
  ...
  if(figma.currentPage.children.some(n=>n.name==='SHV7 / Screens'))throw new Error('현재 페이지에 SHV7 / Screens가 이미 있습니다. 중복 생성을 막았습니다. 새 빈 페이지에서 실행하거나 기존 결과를 사용하세요.');
  ...
  library=track(figma.createFrame());library.name='SHV7 / Components';library.resize(1800,1900);library.x=maxX+100;library.y=100;library.fills=[paint('#F7F5F2')];library.clipsContent=false;
  await createIconMasters();
  root=track(figma.createFrame());root.name='SHV7 / Screens';root.layoutMode='VERTICAL';root.primaryAxisSizingMode='AUTO';root.counterAxisSizingMode='AUTO';root.itemSpacing=64;root.paddingTop=32;root.paddingBottom=32;root.paddingLeft=32;root.paddingRight=32;root.fills=[paint('#E7E9EF')];root.x=maxX+2020;root.y=100;
  ...
  const darkLinks={home:'05-home-dark',tasks:'18-tasks-dark',data:'19-data-dark',settings:'20-settings-dark','reboot-confirm':'22-reboot-confirm-dark'};
  for(const item of actionNodes){let dst=screenNodes.get(item.theme==='dark'?(darkLinks[item.action]||links[item.action]):links[item.action]);if(!dst||!item.node.setReactionsAsync)continue;try{await item.node.setReactionsAsync([{trigger:{type:'ON_CLICK'},actions:[{type:'NODE',destinationId:dst.id,navigation:'NAVIGATE',transition:null,preserveScrollPosition:false}]}]);}catch(e){warnings.push('Prototype link '+item.action+': '+String(e));}}
 ...
 }catch(e){if(root)root.name='SHV7 / PARTIAL — '+stage;figma.ui.postMessage({type:'error',message:String(e),stage,createdNodes:allCreated.length});}finally{busy=false;}
}
