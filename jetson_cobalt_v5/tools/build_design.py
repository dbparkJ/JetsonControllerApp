...
for tone,(bg,fg,bd) in {
 'primary':('primary','onPrimary',None),'accent':('accent','onAccent',None),'secondary':('surface','ink','border'),
 'heroSecondary':('hero','heroText','heroMuted'),'danger':('danger','onPrimary',None),
...
def nav(selected):
 ...
 for key,label,ico in [('home','홈','home'),('tasks','작업','play'),('data','데이터','folder'),('settings','설정','settings')]:
  d=V('Nav/'+key,H('SelectedIndicator',I(ico,'onAccent' if selected==key else 'muted',22),w=58,h=32,bg='accent' if selected==key else None,radius=16,justify='center'),T(label,12,700 if selected==key else 500,'ink' if selected==key else 'muted'),align='center',gap=4,w='fill',minH=56)
...
def taskcard(title,status,sub,kind='normal',action='task-detail'):
 ...
 actions=H('TaskActions',B('상태 보기','task-detail','accent'),C('Button/heroSecondary','중지','stop-confirm',w=86),gap=10,w='fill') if kind=='running' else B('시작 요청 중…',None,'disabled') if kind=='pending' else B('문제 확인','task-failure','secondary')
...
def tasks():
 return [H('TaskFilters',badge('전체 3','neutral'),badge('실행 중 1','success'),badge('확인 필요 1','warning'),w='fill',gap=8),taskcard('야간 라인 검사','실행 중 · 42분','마지막 결과 02:14 · 파일 128개 저장','running'),taskcard('정밀 측위 로그','시작 요청 중','요청을 보냈습니다. 장비의 실행 확인을\n기다리고 있습니다.','pending'),taskcard('표면 결함 수집','실패','장비 인증을 확인하지 못해 시작하지 못했습니다.','failure')]
...
spec={'version':'5.0','name':'Jetson Controller / Cobalt Field V5','created':'2026-09-08','status':'local-prototype-not-native-app','screenSize':{'width':412,'height':892},'font':{'figma':'Noto Sans KR','local':'Noto Sans CJK KR','android':'FontFamily.SansSerif'},'themes':THEMES,'icons':ICONS,'components':COMPONENTS,'screens':SCREENS}
...
for i,f in enumerate(files,1):
 src=Path('/mnt/data')/f; dest=P/'reference'/f'{i:02d}_user_reference.png';shutil.copy(src,dest) if src.exists() else None
