...
class MobileRtkRelayRegistry:
    ...
    def _atomic_write(self, value: Dict[str, object]) -> None:
        ...
        try:
            ...
            try:
                # jetson-control-api.service runs with UMask=0077, while the
                # managed pipeline may run as an unprivileged user. This file
                # contains only a short-lived host/port route, never NTRIP
                # credentials, so make it explicitly readable after creation.
